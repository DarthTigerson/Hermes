...
async def edit_employee(request: Request, employee_id: int, db: Session = Depends(get_db)):
    ...
    if role_state.payroll == True:
        employee_contract = await get_employee_contract(request=request, employee_id=employee_id, db=db)
    else:
